...
class MainActivity : AppCompatActivity() {
    ...
    private fun initData() {
         ...
         * */
        IMHelper.addReceiveObserver<String>("MainActivity", this) //
            ...
            .withData(queryData) // 参考 1.1 withData(obj)
         ...
         * */
        IMHelper.addTransferObserver<String, Int>("MainActivity", this) //
            .transform(TestMsgTransfer::class.java)  // 使用哪个类处理消息的转换，可参考 DataHandler<T>
            .filterOut { s, pl -> //经过 DataHandler 处理后的数据，返回 false 则会丢弃。
                Log.e("---- ", "filterOut --> \ns = $s  \npl = $pl")
                ...
                /**
                 * @param r 单个数据 ：Int
                 * @param lr 列表数据 List<Int>
                 * @param pl 操作指令
                 * */

                Log.e("---- ", "addTransferObserver : new receive --> \nr = $r   \nlr = $lr   \npl = $pl")
            }
    }
}
